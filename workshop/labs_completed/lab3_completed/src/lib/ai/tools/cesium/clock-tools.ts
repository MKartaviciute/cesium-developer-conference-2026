...
export function createClockTools(viewerRef: RefObject<Viewer | null>) {
  return {
    setTime: tool({
      description:
        "Jump the scene clock to a specific date and time (ISO 8601 format). Use for simple point-in-time requests like 'set the time to', 'change the date to', 'go to [date]', 'simulate [historical event date]'. Prefer clockControl when playback speed, loop mode, or time window also need to change.",
    ...
    getViewerState: tool({
      description:
        "Get a full snapshot of the CesiumJS viewer: camera position, all visible entities, loaded data sources, imagery layers, and scene clock time. Call this proactively before broad operations ('what's on the globe?', 'show me everything', 'clear the scene') or when you need to know what is already loaded before making changes.",
    ...
    clockControl: tool({
      description:
        "The preferred tool for all clock operations — use this over setTime for most requests. Use action='setTime' to jump to a specific datetime ('set clock to', 'travel to [date]', 'simulate [event]'). Use action='setMultiplier' for playback speed ('speed up time', 'slow down', 'fast-forward', '10× speed'). Use action='configure' to set multiple properties at once (time window, loop mode, animation on/off).",
