...
import { registerWorldBankTools } from "./tools/worldbank-tools.js";
...
app.post("/mcp", mcpLimiter, async (req, res) => {
  const server = new McpServer({ name: "worldbank", version: "1.0.0" });
...
app.listen(PORT, () => {
  console.log(`World Bank MCP server running on http://localhost:${PORT}/mcp`);
});
