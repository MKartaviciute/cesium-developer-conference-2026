...
import { registerFirmsTools } from "./tools/firms-tools.js";
...
app.post("/mcp", mcpLimiter, async (req, res) => {
  const server = new McpServer({ name: "firms", version: "1.0.0" });
...
app.listen(PORT, () => {
  console.log(`FIRMS MCP server running on http://localhost:${PORT}/mcp`);
});
