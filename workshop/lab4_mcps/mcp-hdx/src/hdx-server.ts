...
app.post("/mcp", mcpLimiter, async (req, res) => {
  const server = new McpServer({ name: "hdx", version: "1.0.0" });
  registerHdxTools(server);
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 3023;
app.listen(PORT, () => {
  console.log(`HDX MCP server running on http://localhost:${PORT}/mcp`);
});
