...
import { registerTigerwebTools } from "./tools/tigerweb-tools.js";
...
app.post("/mcp", mcpLimiter, async (req, res) => {
  const server = new McpServer({ name: "tigerweb", version: "1.0.0" });
  registerTigerwebTools(server);
...
app.listen(PORT, () => {
  console.log(`TIGERweb MCP server running on http://localhost:${PORT}/mcp`);
});
